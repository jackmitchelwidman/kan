...
### Design Document — Version 0.10.0 (2026-08-09 22:31 IDT)
...
14. **The Kan lens — every program is a Kan extension** (`kan explain`) — a program is a *presentation*: each definition is a generator, and the program's meaning is its **initial model**, a left Kan extension of the generators along their inclusion. `kan explain <file>` renders each definition as its **fiber** of that one construction — folds/matches as the rich fibers (genuine (co)limits / Kan extensions), a literal as the trivial fiber. This makes the philosophy (*"a Kan program is a partial diagram awaiting universal completion"*) something the compiler *shows* on real code, uniformly, degenerate cases and all. The flip side is the **universality checker**: the structural-recursion/totality checker (which already accepts exactly the *folds* — the unique maps out of initial algebras) now *narrates its rejections in universal terms* — "a total function is the fold, the unique Kan extension of its cases, so recurse on a constructor sub-part / cover every constructor / keep the recursion in tail position / carry extra state as an accumulator" — turning "this isn't total" into "this isn't yet the canonical universal map; here's how to make it one."
15. **"Recursion is a Kan extension," proven in Kan** ([`examples/universal.kan`](examples/universal.kan)) — the philosophy as a *checked theorem*, not a slogan. A ℕ-algebra is the data of a fold; `fold` is the canonical extension (existence); `fold_unique` proves — by induction, not `refl` — that *any* map matching the algebra equations equals the fold (the **universal property**: the fold is the unique map extending the generators, i.e. the left Kan extension along `{zero, suc} ↪ ℕ`); and `add_is_the_kan_extension` shows the ordinary recursive `add` *is* such a fold, hence the unique universal map. So a real recursive function is exhibited, with a machine-checked proof, as a left Kan extension.
16. **`fill` — the universal completion, as one interface** ([`examples/fill.kan`](examples/fill.kan)) — the operational form of the philosophy. A `record Universal { Horn, Cand, Good, EqC, fill, fillGood, fillUniq }` packages a partial diagram, its candidate completions, the property a completion must satisfy, an equivalence, and the three pieces of universality (`fill` = the completion, `fillGood` = existence, `fillUniq` = uniqueness). *Different* universal constructions become the same `fill`: recursion (the fold — reusing the `universal.kan` theorem, so `fill FoldUniversal (ℕ,2,suc) 5 = 7`) and the terminal object (`fill TerminalUniversal`), both checked, all three runtimes agreeing.
17. **`?` — the compiler fills the horn** ([`examples/holes.kan`](examples/holes.kan)) — the operational north star, Stage 2a. A hole `?` in check-mode is filled with the **unique universal completion** when its goal type is *contractible* — `Unit → unit`, `Σ → a pair`, `Π → a lambda`, `Id A a a → refl`, a single-constructor datatype → its point — and the fillers compose (`? : (A:U) -> A -> Unit` becomes `λA a. unit`). When the goal has several inhabitants (`Nat`, `Bool`, coproducts) it **refuses and reports the goal** `⊢ ? : T` — never a guess. Sound, deliberately incomplete, *contractible-only* (fill iff the completion is universal). Adds a `Hole` to the core and a `fill` recognizer in `check`; `kan run`/`build` now type-check (which is what fills the holes). Next: dispatch `?` to registered `Universal` instances (needs implicit arguments). See [`docs/fill-elaborator.md`](docs/fill-elaborator.md).
