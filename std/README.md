...
| `creal.kan` | **constructive reals** (complete) `CReal = Nat -> Rational` (`x n` within `1/2^n` of the value): a real is a *process* producing rational approximations. `addC`/`subC`/`negC`/`mulC`/`absC` with precision propagation, `recipC`/`divC` gated by an apartness-from-zero witness (`Apart0`), and the constants `sqrt2`, `piC` (Machin), `eC`; read digits with `scaledFloor x d = ⌊x·10^d⌋`. Computes fast (on the primitive-`Integer` rational). No `eqC` (real equality is undecidable). See ADR-017. |
